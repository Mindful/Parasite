...
public class dayNightCycle : MonoBehaviour
{
	...
	//public GameObject[] spawnPoints;//an array of the available spawn points on the map
	
	//public GameObject player;
	
	public bool Underground = false; //are we indoors or in a cave somewhere?
    ...
    void Update()
    {
        // Rudimentary phase-check algorithm:
		if (!Underground)
		{
	        if (currentCycleTime > nightTime && currentPhase == DayPhase.Dusk)
	        {
	            SetNight();
//				if (rotate)
//				player.SendMessage("SwitchTracks",1);
				//player.GetComponent("MusicManager").SwitchTracks(1);
	        }
	        ...
	        else if (currentCycleTime > dayTime && currentPhase == DayPhase.Dawn)
	        {
	            SetDay();
//				if (rotate)
//				player.SendMessage("SwitchTracks",0);
				//player.GetComponent<MusicManager>().SwitchTracks(0);
	        }
		...
		else
		{
			SetNight();	
//			if (rotate)
//			player.SendMessage("SwitchTracks",2);
			//player.GetComponent<MusicManager>().SwitchTracks(2);
		}
...
//		
//	}
	//Generate a list of available spawn points that do not have 
	//any mobs childed to it.
...
//		return gos.ToArray();
//	}
//	public void SetUnderground(bool u)
//	{
//		Underground = u;	
//	}
