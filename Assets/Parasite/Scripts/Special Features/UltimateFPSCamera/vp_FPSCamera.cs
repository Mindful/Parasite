...
{
	private int delayedPivotVisibleTimer;
	private vp_FPSWeapon pivotWeapon;
		...
		if (networkView.isMine)
 		{
			...
			if (delayedPivotVisibleTimer > 0)
			{
				delayedPivotVisibleTimer--;
				
				if (delayedPivotVisibleTimer == 0)
					pivotWeapon.SetPivotVisible(false);
				SetWeapon(2);
				SetWeapon(1);
			}
			UpdateMouseLook();
	...
	public void SwitchWeapon(int weapon, WeaponSwitchDelegate callback = null)
	{
		...
		if (CurrentWeapon == null)
			t_Wait = 0.50f;
		m_SwitchWeaponTimer = vp_Timer.In(t_Wait, delegate()
		{
	...
	public void SetWeapon(int i)
	{
		...
		if (m_CurrentWeaponID > 0)
		{
			...
			if (m_CurrentWeapon != null)
			{
				...
				if(m_CurrentWeapon.GetPivot() != null)
				{
				...
				else
				{
					...
					delayedPivotVisibleTimer = 30;	
					pivotWeapon = m_CurrentWeapon;
				}
