...
public class vp_FPSShooter : vp_Component
{
	...
	public void Fire()
	{
		...
		if (audio != null)
		{
			networkView.RPC("PlayAudio",RPCMode.AllBuffered);
...
//			audio.Play();
		...
		for (int v = 0; v < ProjectileCount; v++)
		{
			if (ProjectilePrefab != null)
			{
				...
				//p = Network.Instantiate(ProjectilePrefab, m_Camera.transform.position, m_Camera.transform.rotation,0)as GameObject;
