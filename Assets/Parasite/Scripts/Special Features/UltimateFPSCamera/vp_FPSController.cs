...
public class vp_FPSController : vp_Component
{
		...
		if (networkView.isMine)
 		{
			...
			if (idleTimer > 0)
			{
				idleTimer--;
				if (idleTimer == 0)
					networkView.RPC("AnimateIdle",RPCMode.AllBuffered);	
			}
			...
			if (m_Controller.isGrounded && !m_WasGroundedLastFrame)
			{
				...
				networkView.RPC("JumpLandSound",RPCMode.AllBuffered);
	...
	public void SetParaForm(bool p)
	{
		paraForm = p;
	}
	...
	protected void UpdateMoves()
	{
		...
		{m_MoveFactor = 1.0f;}
        m_MoveFactor *= pc.get_movementMultiplier();
		...
		if (m_MoveForward)
		{
			...
			networkView.RPC("AnimateWalk",RPCMode.AllBuffered,1);
			idleTimer = 0;
		}
		if (m_MoveBack)
		{
		...
		if (!m_MoveForward && !m_MoveBack && !m_MoveRight && !m_MoveLeft)
		{
			idleTimer = 10;
		}
		...
		if (climb)
		{
			m_MotorThrottle+=transform.TransformDirection(Vector3.up *  5.0f * (MotorAcceleration *0.2f) * m_MoveFactor);	
		}
	...
	public bool Jump()
	{
		float MotorJumpForce_old = MotorJumpForce;
		if (paraForm)
		{
		...
		MotorJumpForce = MotorJumpForce_old;
		networkView.RPC("JumpSound",RPCMode.AllBuffered);
		return true;
