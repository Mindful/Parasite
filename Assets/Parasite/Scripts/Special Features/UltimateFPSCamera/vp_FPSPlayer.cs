...
public class vp_FPSPlayer : MonoBehaviour
{
	...
	public bool m_LockCursor = true;						// enables / disables the mouse cursor
...
//			InputCycleWeapon();
	...
	public void SetParasiteForm(bool p)
	{
		parasiteForm = p;	
	}
		...
		if (Input.GetMouseButton(0))
		{
			if (!parasiteForm)
			{
				if (CurrentWeapon != null)
				{
					...
					{
						if (meleeCooldown < 1 && fireButtonReleased)
						{
							CurrentWeapon.animation["Attack"].speed = 1f;
							CurrentWeapon.animation.Play("Attack", PlayMode.StopAll); 
							//meleeCooldown = CurrentWeapon.meleeCooldown;
							meleeCooldown = (int)(CurrentWeapon.animation["Attack"].length);
							fireButtonReleased = false;
							audio.PlayOneShot(CurrentWeapon.meleeAttackSounds[Random.Range(0,CurrentWeapon.meleeAttackSounds.Length)]);
							...
							foreach(Collider c in targets)
							{
								if (c != gameObject.collider)
								{
									...
									{
										if(c.gameObject.GetComponent<healthObject>())
										{
										healthObject damageTarget = c.gameObject.GetComponent<healthObject>();
						                  damageTarget.applyDamage(damageTarget, (int)CurrentWeapon.meleeDamage); //CURRENTLY DAMAGING SELF
										//  damageTarget.applyStatus<debuff_dot>(damageTarget);
										audio.PlayOneShot(CurrentWeapon.meleeImpactSounds[Random.Range(0,CurrentWeapon.meleeImpactSounds.Length)]);
										}
									}
