...
//			{
			  if(hit.collider is SphereCollider)
				{
			  		//hit.collider.transform.SendMessage("ApplyDamage",Damage*3);
					if (hit.collider.gameObject.networkView)
				 	{
                        healthObject target = hit.collider.gameObject.GetComponent<healthObject>();
                        target.applyDamage(target, (int)Damage * 3); //CURRENTLY DAMAGING SELF
					//	target.applyStatus<buff_speed>(target);
                    }
				} 
				else
				{
					//hit.collider.transform.SendMessage("ApplyDamage",Damage);
					if (hit.collider.gameObject.networkView && hit.collider.gameObject.GetComponent<healthObject>())
					{
                        healthObject target = hit.collider.gameObject.GetComponent<healthObject>();
                        target.applyDamage(target, (int)Damage); //CURRENTLY DAMAGING SELF
                    }
				}
			...
			if (m_ImpactPrefab != null)
				Network.Instantiate(m_ImpactPrefab, transform.position, transform.rotation,0);

			// spawn dust effect
			if (m_DustPrefab != null)
				Network.Instantiate(m_DustPrefab, transform.position, transform.rotation,0);

			// spawn spark effect
			if (m_SparkPrefab != null)
			{
				if (Random.value < m_SparkFactor)
					Network.Instantiate(m_SparkPrefab, transform.position, transform.rotation,0);
			}

			// spawn debris particle fx
			if (m_DebrisPrefab != null)
				Network.Instantiate(m_DebrisPrefab, transform.position, transform.rotation,0);
			...
			Network.Destroy(gameObject);	// hit nothing, so self destruct immediately
